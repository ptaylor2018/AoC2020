...
def part1():
    input_raw = []
    with open("small_input_day18.txt", "r") as reader:
...
def compute_answer(line: str) -> int:
    if contains_no_parens(line):
        ...
        return compute_answer(prefix + compute_answer(seg) + postfix)
...
def sep_paren_seg(line: str) -> [str, str, str]:
    ...
    close_loc = 0
    level_count = 0
    for i in range(open_loc+1,len(line)):
        if line[i] == '(':
            level_count+=1
        if line[i] == ')':
            level_count-=1
